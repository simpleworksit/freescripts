...
{
	$volume = Get-CimInstance -Class Win32_Volume | Where-Object { $partitionPath -contains $_.DeviceID }
...
{
    if ($item -match 'harddisk(\d+)')
	{
		$OSDiskIndex = $matches[1]
	}
	if ($item -match 'partition(\d+)')
	{
		$WinREPartitionIndex = $matches[1]
	}
}
